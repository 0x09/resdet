...
int setup_analysis_group(void** state) {
	...
	RDError err = resdet_read_image("test/files/blue_marble_2012_resized.pfm",NULL,&ctx->image,&nimages,&width,&height);
	if(err)
		return 1;
...
void test_analyze_image_with_no_analysis_returns_error(void** state) {
	...
	RDError err = resdet_analyze_image(NULL,ctx->image);
...
void test_analyze_image_with_no_image_returns_error(void** state) {
	...
	RDError err = resdet_analyze_image(ctx->analysis,NULL);
...
void test_analysis_results_with_no_analysis_returns_error(void** state) {
	...
	RDError err = resdet_analysis_results(NULL,&resw,&countw,&resh,&counth);
...
void test_analysis_results_with_no_images_returns_error(void** state) {
	...
	RDError err = resdet_analysis_results(ctx->analysis,&resw,&countw,&resh,&counth);
...
void test_analysis_results_with_resw_but_no_countw_returns_error(void** state) {
	...
	RDError err = resdet_analysis_results(ctx->analysis,&resw,NULL,&resh,&counth);
...
void test_analysis_results_with_resh_but_no_counth_returns_error(void** state) {
	...
	RDError err = resdet_analysis_results(ctx->analysis,&resw,&countw,&resh,NULL);
