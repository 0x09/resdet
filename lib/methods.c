...
static RDError detect_method_sign(const float* restrict f, size_t length, size_t n, size_t stride, size_t dist, RDResolution**detect, size_t* count, size_t range, float threshold) {
...
static RDError detect_method_magnitude(const float* restrict f, size_t length, size_t n, size_t stride, size_t dist, RDResolution**detect, size_t* count, size_t range, float threshold) {
	if(!(*detect = malloc(sizeof(**detect)*(*count+1)))) return RDENOMEM;
	**detect = (RDResolution){length,-1}; *count = 1;
	if(range*2 >= length)
		return RDEOK; //can't do anything
	...
	for(rdint_index x = range; x < length-range; x++) {
		...
		for(rdint_index y = 0; y < n; y++)
			for(rdint_index i = 1; i <= range; i++) {
				int e;
				frexpf(f[y*stride+x*dist-i*dist]/copysignf(MAX(fabsf(f[y*stride+x*dist+i*dist]),FLT_EPSILON),f[y*stride+x*dist+i*dist])+1,&e);
				sign += e <= 0;
			}
		if(sign > range*n*threshold) {
			if(!(*detect = realloc(*detect,sizeof(**detect)*(*count+1)))) return RDENOMEM;
...
static RDError detect_method_original(const float* restrict f, size_t length, size_t n, size_t stride, size_t dist, RDResolution**detect, size_t* count, size_t range, float threshold) {
#ifndef MAG_RANGE
#define MAG_RANGE range
	...
	**detect = (RDResolution){length,-1}; *count = 1;
	...
	RDError ret = RDEOK;
	float* line = NULL,* sum = NULL; rdint_storage* sign = NULL;
	...
	for(rdint_index x = maxrange; x < length-maxrange; x++) {
		long double left = 0, right = 0, mid = sum[x] * MAG_RANGE;
		for(rdint_index i = 1; i <= MAG_RANGE; i++) {
			left += sum[x-i];
			right += sum[x+i];
		}
		int leftexp, rightexp, midexp;
		frexpl(left,&leftexp); frexpl(right,&rightexp); frexpl(mid,&midexp);
		if((abs(leftexp - rightexp) < 2) && (!mid || MIN(leftexp,rightexp) >= midexp) && (MIN(left,right) > mid) && (fabs(left - right) < fabs(left - mid) && fabs(left - right) < fabs(right - mid)) && 
...
static RDMethod methods[] = {
	{
		.name = "sign",
		.func = detect_method_sign,
		.threshold = 0.55
	},{
		.name = "mag",
		.func = detect_method_magnitude,
		.threshold = 0.40
	},{
		.name = "orig",
		.func = detect_method_original,
		.threshold = 0.64
	},
	{}
};
