...
static RDError detect_method_sign(const coeff* restrict f, size_t length, size_t range, intermediate* result, rdint_index* restrict start, rdint_index* restrict end) {
	for(rdint_index x = *start; x < *end; x++) {
		...
		result[x-*start] += sign_diff / (intermediate)(range);
	}
	return RDEOK;
}

// Looks for similar magnitude coefficients with inverted signs.
static RDError detect_method_magnitude(const coeff* restrict f, size_t length, size_t range, intermediate* result, rdint_index* restrict start, rdint_index* restrict end) {
	for(rdint_index x = *start; x < *end; x++) {
		...
		result[x-*start] += mag_match / (intermediate)(range);
	}
	return RDEOK;
}

// not quite identical to original method, but closest linear equivalent
static RDError detect_method_original(const coeff* restrict f, size_t length, size_t range, intermediate* result, rdint_index* restrict start, rdint_index* restrict end) {
#ifndef MAG_RANGE
#define MAG_RANGE range
	...
	for(rdint_index x = *start; x < *end; x++) {
		...
		if(MIN(left,right) > mid && lrdiff < mi(fabs)(left-mid) && lrdiff < mi(fabs)(right-mid)) {
			...
			result[x-*start] += sign / (intermediate)(range);
		}
	}
	return RDEOK;
}
...
static RDError detect_method_zerocrossing(const coeff* restrict f, size_t length, size_t range, intermediate* result, rdint_index* restrict start, rdint_index* restrict end) {
	for(rdint_index x = *start; x < *end; x++) {
